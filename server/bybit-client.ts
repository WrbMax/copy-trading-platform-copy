...
/**
 * Get Bybit unified account wallet balance (USDT).
 * Uses /v5/account/wallet-balance with accountType=UNIFIED.
 */
export async function getBybitBalance(
  ...
  }>(creds, "GET", "/v5/account/wallet-balance", { accountType: "UNIFIED" });
  ...
  try {
    const data2 = await bybitRequest<{
      list: Array<{ totalWalletBalance: string; totalAvailableBalance: string }>;
    }>(creds, "GET", "/v5/account/wallet-balance", { accountType: "CONTRACT" });
    const acc2 = data2?.list?.[0];
    return {
      totalWalletBalance: acc2?.totalWalletBalance || "0",
      availableBalance: acc2?.totalAvailableBalance || "0",
    };
  } catch {
