...
async function executeCopyTrades(sourceId: number, change: PositionChange) {
  ...
  async function executeForUser(us: typeof userStrategies[0]): Promise<boolean> {
    ...
    if (isCloseAction && exchangeOrderId) {
      ...
      try {
        ...
        // Find ALL matching open orders for this user/symbol/side (filtered by signalSourceId)
        const allOpenOrders = await findAllUserOpenOrders(us.userId, change.instId, change.action, sourceId);

        // ── Distribute PnL across ALL open orders ──
        // Use exchange-provided realizedPnl, distribute proportionally by qty.
        // This ensures PnL matches what users see in their exchange account.
        //
        // For reduce_long/reduce_short (partial close): only close orders up to the
        // traded quantity in FIFO order. Remaining open orders stay open with updated quantity.
        let rawPnl = realizedPnl;

        const isReduceAction = change.action === "reduce_long" || change.action === "reduce_short";

        if (allOpenOrders.length > 0) {
          // Calculate total quantity across all open orders
          const totalOpenQty = allOpenOrders.reduce((sum, o) => sum + parseFloat(o.actualQuantity || "0"), 0);

          // For partial close: the close qty is proportional to signal contractsDelta vs total signal position
          // We close orders in FIFO order until we've consumed the close qty
          // For full close: close all open orders
          let remainingCloseQty = isReduceAction
            ? Math.min(parseFloat(ethQty.toFixed(8)), totalOpenQty) // close up to actualQty of this close order
            : totalOpenQty; // full close: close all

          for (const openOrd of allOpenOrders) {
            if (remainingCloseQty <= 0) break;

            const ordQty = parseFloat(openOrd.actualQuantity || "0");
            const closeQty = Math.min(ordQty, remainingCloseQty);
            remainingCloseQty -= closeQty;

            const ratio = totalOpenQty > 0 ? closeQty / totalOpenQty : 1 / allOpenOrders.length;
            const ordRawPnl = rawPnl * ratio;
            const ordFee = fee * ratio;
            const ordNetPnl = ordRawPnl - ordFee;

            if (closeQty >= ordQty - 0.000001) {
              // Fully closing this open order
              ...
              console.log(`[CopyEngine] 📊 User ${us.userId} order ${openOrd.id}: FULL close, ratio=${ratio.toFixed(4)}, netPnl=${ordNetPnl.toFixed(4)}`);
            } else {
              // Partially closing this open order — update remaining qty, keep status open
              const remainingQty = ordQty - closeQty;
              await updateCopyOrder(openOrd.id, {
                actualQuantity: remainingQty.toFixed(8),
                // Record partial PnL in closePrice/realizedPnl for display, but keep open
                closePrice: closePrice.toFixed(8),
                realizedPnl: ordRawPnl.toFixed(8),
                fee: ordFee.toFixed(8),
                netPnl: ordNetPnl.toFixed(8),
              });
              console.log(`[CopyEngine] 📊 User ${us.userId} order ${openOrd.id}: PARTIAL close ${closeQty.toFixed(8)}/${ordQty.toFixed(8)}, remaining=${remainingQty.toFixed(8)}`);
            }
          }
        }
        ...
        // Do NOT overwrite openPrice — it was set from signal avgPx and is the entry reference
        await updateCopyOrder(orderId, {
          closePrice: closePrice.toFixed(8),
        ...
        // Auto-award Route points for net loss orders (1 USDT loss = 1 Route)
        // Only triggered by API signal close orders (this code path is signal-driven)
        if (netPnl < 0) {
          try {
            // 1 USDT loss ≈ 1 Route (rounded to nearest integer)
            const routeAmount = Math.round(Math.abs(netPnl));
            if (routeAmount > 0) {
              const user = await getUserById(us.userId);
              if (user) {
                const newPoints = await adjustPoints(us.userId, routeAmount);
                await addPointsTransaction({
                  userId: us.userId,
                  type: "redeem",
                  amount: routeAmount,
                  balanceAfter: newPoints,
                  note: `交易亏损自动发放 Route（亏损 ${Math.abs(netPnl).toFixed(4)} USDT，发放 ${routeAmount} Route）`,
                });
                console.log(`[CopyEngine] 🎯 Route awarded to user ${us.userId}: ${routeAmount} Route (loss=${Math.abs(netPnl).toFixed(4)} USDT)`);
              }
            }
          } catch (routeErr: unknown) {
            const routeMsg = routeErr instanceof Error ? routeErr.message : String(routeErr);
            console.error(`[CopyEngine] ⚠️ Route award failed for user ${us.userId}: ${routeMsg}`);
          }
        }
  ...
  for (let i = 0; i < userStrategies.length; i += BATCH_SIZE) {
    ...
    const batchResults = await Promise.allSettled(
      batch.map(async (us) => {
        try {
          const result = await executeForUser(us);
          return result;
        } catch (err: unknown) {
