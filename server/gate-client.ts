...
/**
 * Get Gate.io USDT perpetual futures account balance.
 * Uses /api/v4/futures/usdt/accounts.
 */
export async function getGateBalance(
  ...
  const data = await gateRequest<{
    total: string;
    available: string;
  }>(creds, "GET", "/api/v4/futures/usdt/accounts", {});
  return {
    totalWalletBalance: data?.total || "0",
